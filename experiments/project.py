...
SUITE_SATISFICING = [
    ...
    "ged-sat14-strips", "grid","grid-axioms", "gripper", "hiking-sat14-strips",
...
# === Added by assistant: include additional axiom-heavy benchmarks ===
# These are the remaining axiom benchmarks from the provided list that were not yet present.
_AXIOM_BENCHMARKS = [
    "acc-cc2-ghosh-etal",
...
# Add them to the satisficing suite (only if not already included).
for _d in _AXIOM_BENCHMARKS:
    if _d not in SUITE_SATISFICING:
        SUITE_SATISFICING.append(_d)

# Ensure DOMAIN_GROUPS has a group entry for each of these domains.
# We map each domain to a group of the same name, and that group contains the domain name.
for _d in _AXIOM_BENCHMARKS:
    if _d not in DOMAIN_GROUPS:
        DOMAIN_GROUPS[_d] = [_d]

# Rebuild DOMAIN_RENAMINGS to reflect the new DOMAIN_GROUPS entries.
DOMAIN_RENAMINGS.update({})
for _group_name, _domains in DOMAIN_GROUPS.items():
    for _domain in _domains:
        DOMAIN_RENAMINGS[_domain] = _group_name
for _group_name in DOMAIN_GROUPS:
    DOMAIN_RENAMINGS[_group_name] = _group_name
# === End assistant changes ===
